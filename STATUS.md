...
- [~] Stage 3 — 절대 위치 엔진 + 다중 채팅 + 열/사이드 자동 전환 `v0.3.0` (릴리스됨, 열 모드 렌더 검증 대기)
...
1. **열 모드 렌더 검증** — 가로 화면(MacBook Air M1)에서 확인 예정.
   기본 해상도 1440×900이면 3분할은 열 폭 477로 열 모드, 4분할은 357이라 사이드로 내려간다.
   4분할도 열 모드로 쓰려면 `minColumnWidth`를 355 정도로 낮춘다.
