...
**먼저 위험한 쪽을 쟀다.** mul.live 는 SPA 라 주소를 건드리면 라우터가 반응해 플레이어가
다시 로드될 수 있고, 그러면 아이디어 자체가 무의미하다. `history.replaceState` 와
`location.hash` 직접 대입 **양쪽 모두 로드 이벤트 0건**, `src` 동일, iframe 이 DOM 에 그대로 남았다.
해시를 쓴다 — 쿼리스트링은 서버로 가고 경로 파싱과 엮일 여지가 있다.

형식은 `mlpp=1;l=s;w=ht;m=2;o=201;p=v(0,h(1,2))` (39자). 눈으로 읽히게 뒀다.
왕복 일치 확인, 그리고 **깨진 입력 10종을 전부 거절**하는 것도 확인했다 —
빈 문자열, 남의 해시(`#section-2`), 버전 불일치, 방송 수 불일치, 범위 밖 방송,
중복 방송, 괄호 깨짐, 순서 중복, 모드 이상, 뒤에 붙은 찌꺼기.

**채팅 폭을 빠뜨렸다가 실측에서 잡았다.** 처음 형식에는 폭이 없었는데,
`v(0,h(1,2))` 를 실은 링크로 열었더니 칸이 1개로 접혔다. 받는 쪽 채팅 폭이 471px 이라
세로 분할(240×2=480)이 최소 폭에 걸린 것이다. 가드는 제대로 동작했지만 공유자의 의도가 사라진다.
`w` 를 넣고 다시 열어 **방송0 321×889 / 방송1 320×445 / 방송2 320×444** 로 정확히 복원되는 것을 확인했다.
`w` 없는 링크는 받는 쪽 설정을 그대로 쓴다(형식이 생기기 전 링크와의 호환).

**패널에 고급 탭.** `urlSync` 는 `inline` enum 으로 `라벨 [드롭다운]` 한 줄이고,
바로 아래 "레이아웃 링크 복사" 버튼이 붙는다. 탭에 매인 버튼을 붙일 수 있게
`createSettingsPanel` 의 action 에 `tab` 을 받도록 했다.
복사는 `GM_setClipboard` 우선, 실패하면 표준 API 로 떨어진다.
...
**Alt 드래그가 왜 안 됐나.** 키 이벤트는 포커스를 가진 문서에만 간다.
플레이어를 한 번 클릭하면 포커스가 교차 출처 iframe으로 넘어가 부모는 Alt를 영영 못 받는다.
더 나쁘게, 프레임에 포커스가 가는 순간 부모에 `blur` 가 떠서 기존 코드가 교환 모드를 **껐다**.
플레이어를 만지는 순간 기능이 죽는 구조였다.
부모 키 + 부모 `mousemove.altKey` + 프레임 에이전트 보고, 셋을 겹쳐 고쳤다.
`blur` 는 이제 부모 것만 푼다.

**마스터 모드 자리 교환도 깨져 있었다.** 첫 자리는 `order` 가 아니라 `master` 가 정하는데
`swap()` 이 `order` 만 흔들어서 첫 칸은 그대로고 나머지만 엉켰다.
`order` 교환과 `master` 변경을 같이 해야 진짜 자리 교환이 된다.
합성 포인터 이벤트로 전 모드 검증:

| 모드 | 결과 |
|---|---|
| 사이드 | `0,1,2,3` → `2,1,0,3` → `2,3,0,1` |
| 열 | 슬롯 교환 + 채팅이 자기 영상을 따라감 (x = 0/480/960/1440) |
| 수동 격자 2×0 | `0,3,2,1` → `1,3,2,0` |
| 마스터 앤 스택 | 슬롯 0↔2, 0↔1, 2↔3, 0↔3 전부 정확, master·솔로·채팅이 따라감 |

**칸을 숫자에서 트리로.** 수동 분할을 하려면 구조가 남아 있어야 해서 `splitChatPanes`(숫자 하나로
매번 다시 계산)를 버리고 BSP 트리(`panes.js`)로 갈았다. 자동 분할 결과는 예전과 **완전히 같다** —
1~4칸에서 `641x889` 기준 `321x889` / `321x889 + 320x445·444` / 균등 사분할까지 그대로 나온다.
겹침·빈틈·이탈 0, 칸 수 일치 확인.

**떨어뜨릴 자리 판정**(`dropzone.js`)은 10개 지점으로 확인했다 —
바깥 테두리 넷, 메인 가운데/왼쪽/위쪽, 오른쪽 위 칸 가운데/아래, 영역 바깥.
전부 의도한 zone 과 미리보기 사각형이 나왔다.

**우클릭 드래그 배선.** 우클릭이 영상 iframe 안에서 시작해 부모 채팅 영역에서 끝난다.
에이전트 `rcdown` → 부모가 실드 → 부모가 `pointerup`. 실드가 깔리기 전에 손을 떼는 경우를 위해
에이전트도 `rcup` 을 보내고, 먼저 온 쪽이 처리한다(둘 다 와도 두 번 실행되지 않는다).
뗀 뒤 따라오는 `contextmenu` 는 300ms 막는다 — 윈도우는 메뉴가 `mouseup` 뒤에 온다.
**실드가 남으면 페이지 전체가 먹통이 되므로** `pointercancel`·`Escape`·창 `blur` 로도 반드시 푼다.

**채팅 프레임에도 스크립트가 돈다.** 칸 닫기(Shift+우클릭)를 받으려면 채팅 안의 클릭을 봐야 하는데,
부모는 교차 출처라 못 보고 오버레이를 얹으면 스크롤·입력이 다 막힌다.
`@exclude ...vtype=chat...` 을 걷고 `chat-agent.js` 를 태웠다. Shift+우클릭 하나만 가로챈다.
채팅 URL 이 `https://play.sooplive.com/<id>?vtype=chat` 임을 실측하고 `main.js` 가 그것으로 역할을 가른다.

**실사용 확인** — 사용자가 새 빌드로 분할·전환·닫기 동작 확인("잘 돼"). 콘솔 오류 없음.
