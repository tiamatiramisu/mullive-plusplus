...
- [~] Stage 3 — 절대 위치 엔진 + 다중 채팅 + 열/사이드 자동 전환 `v0.3.0` — 열 모드 렌더 검증만 남음
...
1. **열 모드 렌더 검증** — 사용자 환경이 960×1600 세로 모니터라 가로 뷰포트를 만들 수 없다.
   Violentmonkey 메뉴에서 `레이아웃 = 열`로 강제해 확인하거나, 가로 화면에서 확인한다.
2. 확인되면 `v0.3.0` 릴리스
3. Stage 5 (드래그 위치 교환) — 절대 위치라 좌표 교환만 하면 된다
